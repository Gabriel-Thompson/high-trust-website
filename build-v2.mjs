...
const organizationName = 'High Trust Society Foundation';
const meta = {
  index: [organizationName,'A conservative research and advocacy foundation dedicated to rebuilding the conditions that make trust possible.'],
  understand: ['Understand','Study how America became a high-trust society, why trust declined, and where each state stands today.'],
  build: ['Build',`Turn ${organizationName} research into a governing plan that leaders can actually use.`],
  ...
  research: ['Research',`${organizationName} research will study the rise and decline of American high trust and produce practical state-level analysis.`],
  about: ['About',`${organizationName} is an openly conservative research and advocacy organization focused on making American life more worthy of trust.`],
  join: ['Join Us',`Join the people building ${organizationName}: researchers, operators, policy builders, communicators, advocates, volunteers, and supporters.`],
  contact: ['Contact',`Contact ${organizationName} with questions, ideas, media inquiries, partnerships, or other messages.`],
  '404': ['Page Not Found',`The requested ${organizationName} page could not be found.`]
};

const brand = `<a href="/" class="brand" aria-label="${organizationName} home"><img class="brand-logo" src="/assets/high-trust-society-foundation-logo.svg" alt="${organizationName}"></a>`;
const navItems = [['Understand','/understand'],['Build','/build'],['Advocate','/advocate'],['Research','/research'],['About','/about'],['Contact','/contact']];
...
function applyBranding(body){
  return body
    .replaceAll('High Trust America founding prospectus','High Trust Society Foundation founding prospectus')
    .replaceAll('High Trust America research','High Trust Society Foundation research')
    .replaceAll('High Trust America','High Trust Society Foundation');
}
...
for (const file of await readdir(pagesDir)) {
  ...
  const body = applyBranding(cleanBody(source));
  const [title, description] = meta[slug] || [slug,organizationName];
  await writeFile(path.join(out,slug==='index'?'index.html':`${slug}.html`),pageTemplate(slug,title,description,body));
}

console.log(`Built ${Object.keys(meta).length} pages to dist/ for ${organizationName}`);
